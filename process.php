...
$codeid = mysql_insert_id($db);
	...
	$url = addslashes("MECARD:N:".$_POST['vcard'].";TEL:".$_POST['phone'].";ADR:".$_POST['address'].";EMAIL:".$_POST['email'].";;");
...
$codeid = mysql_insert_id($db);
	...
	else if ($_REQUEST['phonenum']) {
	$_POST['phone'] = preg_replace('(\D+)', '', $_POST['phonenum']);
	$url = addslashes("tel:".$_POST['phonenum']);
...
$codeid = mysql_insert_id($db);
	...
	$url = addslashes($_REQUEST['display']);
...
QRcode::png($url, "saved/$fileid.png", "L,M,Q", "10", 2);
...
<p>Right click to download. Don't forget to share, send and post to earn more coins. </p>     <br />
     <div id="step4">   
        ...
        <table><tr>
        <td>
        <a href="javascript:void(0)" onclick="javascript:shareonfb()"><img src="images/share.png" /></a>
        </td>
        <td><a href="javascript:popUp('../email.php')"><img src="images/send.png" /></a>
        </td>
        <td>
        <a href="javascript:void(0)" onclick="javascript:posttowall()"><img src="images/post.png" /></a>
        </td>
             </div>
...
function posttowall() {
	...
	message: 'QReate, share, win. QReate is a social tool that not only helps you share content with your friends via QR codes, but rewards you for it as well. A few simple steps are all it takes to engage and interact on a whole new level.'
	},
	 function(response) {
